 ...
 */
public class JsonMaskImpl implements JsonMask {
     ...
     */
    private JsonMaskImpl(Collection<Filter> filters) {
    ...
    private void applyFilters(DocumentContext jsonContext, Filter filter) {
        ...
        jsonContext.map(filter.getJsonPath(), (currentValue, configuration) -> filter.getMasker().getMapFunction().map(currentValue));
    }
    ...
    public static class Builder {
        ...
        /**
         * Adds a single filter to the Builder, using a MapFunction and JSON path.
         *
         * @param mapFunction The MapFunction to use for the filter.
         * @param jsonPath    The JSON path to apply the filter to.
         * @return The Builder instance, for chaining calls.
         */
        public Builder filter(MapFunction mapFunction, String jsonPath) {
            return filter(Filter.builder().masker(mapFunction).jsonPath(jsonPath).build());
        }
        ...
        /**
         * Adds a single filter or a collection of Filters to the Builder, using a MapFunction and a FiltersBuilder function.
         *
         * @param mapFunction     The MapFunction to use for the filter.
         * @param builderFunction A UnaryOperator that takes a new FiltersBuilder instance.
         * @return The Builder instance, for chaining calls.
         */
        public Builder filter(MapFunction mapFunction, UnaryOperator<FiltersBuilder> builderFunction) {
            assertNotNull(mapFunction);
            return filter(builderFunction.apply(new FiltersBuilder()).masker(mapFunction).build());
        }
         ...
         */
        public JsonMaskImpl build() {
            filters.addAll(filtersBuilders.stream()
                    .map(p -> p.getRight().masker(
                            assertNotNull(maskers.get(p.getLeft()), () -> "Masker " + p.getLeft() + " not defined")
                    ).build())
                    .flatMap(Collection::stream)
                    .collect(Collectors.toList()));
            return new JsonMaskImpl(filters);
        }
        ...
        public static class FiltersBuilder {
            ...
            /**
             * Sets the MapFunction to be used for the filters.
             *
             * @param mapFunction the MapFunction to use
             * @return the builder instance
             */
